...
export default class AboutPage extends Component {
    //region RENDERING
    renderConnection(grade, isBidirectional) {
    ...
    renderDefinition(title, content) {
        return (
    ...
    renderStatementFromSymbols(sen1, grade, isBidirectional, sen2, title) {
    ...
    renderStatement(sen1, grade, isBidirectional, sen2, title) {
        return <div style={{margin:"10px"}}>
            <div style={{fontWeight:"bold"}}>{title}</div>
            <div className="Globals_flexStart" style={{marginTop:"8px"}}>
                {sen1}
