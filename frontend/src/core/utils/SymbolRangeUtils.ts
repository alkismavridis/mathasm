...
export class SymbolRangeUtils {
     ...
     * */
    static addSymbolsToMap(symbolMap:Map<number, MathAsmSymbol>, symbols:MathAsmSymbol[]) {
        if (!symbols || !symbolMap) return;
        symbols.forEach(s => {
            symbolMap.set(s.uid, s);
        });
    }
     ...
     * */
    static addMissingIdsToList(targetIdSet:Set<number>, symbolMap:Map<number, MathAsmSymbol>, symbolIds:number[]) {
        symbolIds.forEach(id => {
            if (!symbolMap.has(id)) targetIdSet.add(id);
        });
    }
     ...
     * The idea is to detect which symbol need to be fetched, and fetch them in one request to the server.
     * */
    static addMissingIdsFromSymbolsToList(targetIdSet:Set<number>, symbolMap:Map<number, MathAsmSymbol>, symbols:MathAsmSymbol[]) {
        symbols.forEach(sym => {
            const id = sym.uid;
            if (!symbolMap.has(id)) targetIdSet.add(id);
        });
    ...
    static addMissingIdsFromStatements(statements:MathAsmStatement[], symbolMap:Map<number, MathAsmSymbol>, target:Set<number>) {
     ...
     * */
    static getMissingIdsFromDirectory(directory:MathAsmDir, symbolMap:Map<number, MathAsmSymbol>) : Set<number> {
        //1. Initialize a set.
        const ret = new Set();
    ...
    static getMissingIdsFromStatements(statements:MathAsmStatement[], symbolMap:Map<number, MathAsmSymbol>) : Set<number> {
        const ret = new Set();
